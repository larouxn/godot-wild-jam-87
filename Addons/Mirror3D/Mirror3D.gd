...
func _process(delta: float) -> void:
	...
	if Engine.is_editor_hint():
		player_camera = (
			Engine.get_singleton(&"EditorInterface").get_editor_viewport_3d().get_camera_3d()
		)
	else:
	...
	if config_dirty:
		...
		quad_material.set_shader_parameter(
			&"mirror_texture_linear", viewport_texture if use_linear_filter else null
		)
		quad_material.set_shader_parameter(
			&"mirror_texture_nearest", viewport_texture if !use_linear_filter else null
		)
	...
	var mirror_transform: Transform3D = get_mirror_transform(
		mirror_normal, mirror_quad.global_position
	)
	...
	var camera_to_mirror_offset: Vector3 = (
		mirror_quad.global_position - mirror_camera.global_position
	)
	...
	var cam_to_mirror_offset_camera_local: Vector3 = (
		mirror_camera.global_basis.inverse() * camera_to_mirror_offset
	)
	var frustum_offset := Vector2(
		cam_to_mirror_offset_camera_local.x, cam_to_mirror_offset_camera_local.y
	)
...
static func get_mirror_transform(normal: Vector3, offset: Vector3) -> Transform3D:
	var basis_x: Vector3 = (
		Vector3(1, 0, 0)
		- (2 * Vector3(normal.x * normal.x, normal.x * normal.y, normal.x * normal.z))
	)
	var basis_y: Vector3 = (
		Vector3(0, 1, 0)
		- (2 * Vector3(normal.y * normal.x, normal.y * normal.y, normal.y * normal.z))
	)
	var basis_z: Vector3 = (
		Vector3(0, 0, 1)
		- (2 * Vector3(normal.z * normal.x, normal.z * normal.y, normal.z * normal.z))
	)
